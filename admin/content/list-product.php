...
if(isset($_REQUEST['did']))
{
    ...
    $imageColumnName = "*";
    $imageTableName = "product_images";
    $imageWhereValue["product_id"] = $_REQUEST['did'];
    $productImagesData = $eloquent->selectData($imageColumnName, $imageTableName, @$imageWhereValue);
...
$columnName["3"] = "product_images.image_name"; // Adjusted to fetch image names
...
$tableName["3"] = "product_images"; // Join with product_images table

$onCondition["1"] = ["products.category_id", "categories.id"];
$onCondition["2"] = ["products.subcategory_id", "subcategories.id"];
$onCondition["3"] = ["products.id", "product_images.product_id"]; // Join condition for images
...
<div class="wrapper">
    ...
    {
        #== CHECK IF PRODUCT HAS BEEN DISPLAYED BEFORE
        if (in_array($eachRow['id'], $displayedProductIds)) {
            continue; // Skip displaying this product if it has been displayed already
        }

        #== GET MAIN PRODUCT IMAGE (Assuming `image_name` is the main image field)
        $productImage = "http://www.placehold.it/200x150/EFEFEF/AAAAAA&text=no+image";
        if (!empty($eachRow['image_name'])) {
            $productImage = $eachRow['image_name'];
        }
        ...
        $displayedProductIds[] = $eachRow['id']; // Add the displayed product ID to the array
        $n++;
    }
?>
